...
VOID SceneFileParser::CrackVertex( BYTE* pDest, const D3DVERTEXELEMENT9* pVertexElements )
{
    WCHAR strSrc[1024];
...
VOID SceneFileParser::ProcessMeshData()
{
    ...
    if( !m_CurrentElementDesc.bEndElement )
    {
        ...
        else if( m_Context.CurrentObjectType == XATG_VERTEXBUFFER )
        {
            ...
            D3DVERTEXELEMENT9 DeclElements[32];
            DWORD dwElementCount = 32;
            D3DVertexDeclaration* pDecl = pVertexData->GetVertexDecl();
            pDecl->GetDeclaration( DeclElements, (UINT*)&dwElementCount );
            DWORD dwVertexSize = D3DXGetDeclVertexSize( DeclElements, 0 );
            ...
            {
                CrackVertex( pVerts, DeclElements );
                pVerts += dwVertexSize;
                m_Context.pUserData = pVerts;
                return;
            }
    ...
    else
    {
        ...
        {
            if( pMesh->GetVertexData(0)->GetVertexStream(0) != NULL && m_Context.pUserData != NULL )
            {
                m_Context.pUserData = NULL;
                pMesh->GetVertexData(0)->GetVertexStream(0)->pVertexBuffer->Unlock();
            }
            m_Context.CurrentObjectType = XATG_MESH;
            return;
        }
        ...
        else if( MATCH_ELEMENT_NAME( L"VertexDecls" ) )
        {
            ...
            pVertexDecl = D3DDevice_CreateVertexDeclaration( pElementList );
            pMesh->GetVertexData(0)->SetVertexDecl( pVertexDecl );
            pVertexDecl->Release();
            delete pElements;
            m_Context.pUserData = NULL;
            return;
...
VOID SceneFileParser::ProcessModelData()
{
    ...
    if( !m_CurrentElementDesc.bEndElement )
    {
        if( MATCH_ELEMENT_NAME( L"Model" ) )
        {
            ...
            const WCHAR* strValue = FindAttribute( L"Mesh" );
            if( strValue != NULL )
            {
                BaseMesh* pMesh = NULL;
                ...
                if( pMesh != NULL )
                {
                    pModel->AddMesh( pMesh, MeshMapping::NoFlags );                    
                }
            }
            return;
        }
